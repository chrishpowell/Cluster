...
public class Tree5a
{
    public static void subtreeDocs(ClusterTree<Cluster> node, Set<Doc> docs, String appender )
    {
        Cluster nc = (Cluster)node.getContents();
        System.out.println(appender + nc +"("+ nc.getScore() +")");
        Set<Doc> sd = nc.getDocs().stream().collect(Collectors.toSet());
        docs.retainAll(sd);
        System.out.println(appender+"  " +sd);
        
        node.getChildren().stream().map(each -> (ClusterTree<Cluster>)each).forEach(each ->  printSimScore((ClusterTree<Cluster>)each, docs, appender + appender));
    }
